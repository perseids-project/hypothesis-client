...
describe HypothesisClient::MapperPrototype do
  ...
  context "attestation of annotation" do 
    ...
    it 'graphed the attestation' do
      expect(mapped[:data]["motivatedBy"]).to eq("oa:linking")
      expect(mapped[:data]["hasBody"]).to be_truthy
      expect(mapped[:data]["hasBody"]["@graph"]).to be_truthy
  ...
  context "relation and attestation" do 
    ...
    it 'graphed the attestation' do
      ...
      expect(mapped[:data]["hasBody"]["@graph"][1]["@id"]).to eq("http://data.perseus.org/people/visiblewords:johndoe_1#this")
      expect(mapped[:data]["hasBody"]["@graph"][1]["snap:has-bond"]).to match_array(["test#bond-1-1"])
      expect(mapped[:data]["hasBody"]["@graph"][2]["@id"]).to eq("test#bond-1-1")
      expect(mapped[:data]["hasBody"]["@graph"][2]["@type"]).to eq("snap:FatherOf")
      expect(mapped[:data]["hasBody"]["@graph"][2]["http://lawd.info/ontology/hasAttestation"]).to be_truthy
      expect(mapped[:data]["hasBody"]["@graph"][2]["snap:bond-with"]["@id"]).to eq("test#rel-target")
      expect(mapped[:data]["hasBody"]["@graph"][3]["@type"]).to eq("http://lawd.info/ontology/Attestation")
      expect(mapped[:data]["hasBody"]["@graph"][3]["http://purl.org/spar/cito/citesAsEvidence"]).to eq("http://example.org")
    end
  end

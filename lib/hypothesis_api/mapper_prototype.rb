...
module HypothesisApi::MapperPrototype
  ...
  class JOTH
    ...
    # @param format expected output format -- only HypothesisApi::FORMAT_OALD supported
    def map(agent,source,data,format)
      ...
      if SMITH_HOPPER_URI.match(data["uri"])
        ...
        if (parts) 
           ...
           model[:sourcePerson] = "#{SMITH_PERSON_URI}#{parts[1]}#{parts[2]}#this" 
      ...
      if (response[:errors].length == 0)
        if (format == HypothesisApi::FORMAT_OALD)
    ...
    def to_oa(obj)
      ...
      oa['hasTarget'] = {
        ...
        "hasSelector" => {
          ...
          "exact" => obj[:TextQuoteSelector]["exact"]
          "prefix" => obj[:TextQuoteSelector]["prefix"]
          "suffix" => obj[:TextQuoteSelector]["suffi"]
        }
      }
      ...
      else if obj[:isCitation]
         oa[:hasBody] = "" # graph of the citation relationships
      else
         oa[:hasBody] = obj[:bodyUri]
      end
      oa
